	...
	DECLARE @start_time DATETIME, @end_time DATETIME, @batch_start_time DATETIME, @batch_end_time DATETIME;
		BEGIN TRY
			...
			SELECT
				...
				sls_cust_id,
				CASE 
					 WHEN sls_order_dt = 0 OR LEN(sls_order_dt) != 8 THEN NULL --Handling the Invalid Data
					 ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE) --Datatype Casting, Changing the datatype.
					 END AS sls_order_dt,
				CASE 
				...
				INSERT INTO silver.erp_loc_a101(
					...
					SELECT 
					REPLACE(cid, '-', '') cid, -- Handled the innvalid values and removed '-'. 
					CASE 
